...
def foward_cg_stab(r1, r4, r2, r3, gait_need, enable):
    if enable == True and (
            abs(r1) + abs(r4) + abs(r2) + abs(r3)) != 0:  # When Key_stap set True in padog, self stab can be use here.
        PA_STABLIZE.get_imu_value()
        if PA_STABLIZE.gyro_cal_sta == 1:  # 等待gyro校准完成
            gyro_p = PA_STABLIZE.filter_data_p
            padog.X_goal = padog.in_y + gait_need + pit_cause_cg_adjust((gyro_p - PA_STABLIZE.p_origin) * pi / 180, 110,
                                                                        1.1) + PA_STABLIZE.gyro_x_fitted * 5
...
def watrlk(t, x_target, z_target, r1, r4, r2, r3):
    ...
    if t >= 4 * Tf:
        ...
        x1 = phase_w_support[0];
        x2 = phase_w_support[0];
        x3 = phase_w_support[0];
        x4 = phase_w_support[0]
        y1 = phase_w_support[1];
        y2 = phase_w_support[1];
        y3 = phase_w_support[1];
        y4 = phase_w_support[1]
